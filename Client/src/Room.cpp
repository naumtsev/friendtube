...
Room::Room(Client *client_, Player *player_, QVector<PlayerView *> &players_,
    ...
    connect(tool_item_right, SIGNAL(set_focus_room()), this,
            SLOT(set_focus_room()));
...
void Room::init_variables() {
    ...
    food.push_back(
            ":/pics/background_item/green_room/more_texture/strawberry.png");
    food.push_back(":/pics/background_item/green_room/more_texture/apple_2.png");
    food.push_back(
            ":/pics/background_item/green_room/more_texture/chicken_leg.png");
    ...
    food.push_back(
            ":/pics/background_item/green_room/more_texture/green_pepper.png");
    ...
    food.push_back(
            ":/pics/background_item/green_room/more_texture/red_pepper.png");
...
void Room::init_video() {
    ...
    video_widget->move(width() / 2 - video_widget->width() / 2,
                       height() / 2 - video_widget->height() / 2);
    video_widget->setStyleSheet("QVideoWidget{background-color : black;}");
    video_player = new VideoPlayer(this, video_widget);
    connect(video_player, SIGNAL(video_request(QJsonObject)), client->n_manager,
            SLOT(video_request(QJsonObject)));
    ...
    connect(video_player->m_player, SIGNAL(stateChanged(QMediaPlayer::State)),
            this, SLOT(set_additional_layer(QMediaPlayer::State)));
    ...
    video_advert->setStyleSheet(
            "QLabel { background-color : #C20015; color : white; }");
    ...
    connect(video_player, &VideoPlayer::make_advert, [this](QString text) {
        ...
        video_advert->show();
        video_advert->move(
                video_widget->x() + video_widget->width() - video_advert->width(),
                video_widget->y() - video_advert->heightForWidth(width_advert) - 3);
        this->set_focus_room();
    });

    // smile
    tool_item_right->show_multicolor_emoji_list_button->setFixedSize(
            video_btn_size, video_btn_size);

    // add video
    push_button_add_video = new QPushButton(this);
    push_button_add_video->setIcon(
            QIcon(QPixmap(":/pics/add_new_video_push_button.png")));
    ...
    push_button_add_video->setGeometry(
            video_widget->x(),
    ...
    push_button_pause_video->setIcon(
    ...
    push_button_pause_video->setGeometry(
            push_button_add_video->x() + video_btn_size + video_btn_space_size,
    ...
    push_button_stop_video->setIcon(
            QIcon(QPixmap(":/pics/delete_video_push_button.png")));
    ...
    push_button_stop_video->setGeometry(
            push_button_pause_video->x() + video_btn_size + video_btn_space_size,
    ...
    volume_slider->setStyleSheet(
            "QSlider::groove:horizontal { border: 1px solid #ffffff; height: 18px; "
            "border-radius: 9px; }QSlider::handle:horizontal {width: "
            "18px;}QSlider::add-page:qlineargradient {background: "
            "lightgrey;border-top-right-radius: 9px;border-bottom-right-radius: "
            "9px;border-top-left-radius: 0px;border-bottom-left-radius: "
            "0px;}QSlider::sub-page:qlineargradient {background: "
            "#42ff42;border-top-right-radius: 0px;border-bottom-right-radius: "
    ...
    volume_slider->move(push_button_stop_video->x() +
                                push_button_stop_video->width() +
    ...
    connect(volume_slider, SIGNAL(valueChanged(int)), video_player,
            SLOT(change_volume(int)));// фокус возращаю в change_volume
    ...
    connect(client->n_manager, SIGNAL(video_pause()), video_player,
            SLOT(pause()));
}
...
void Room::init_buttons() {
    ...
    push_button_exit_in_menu->setStyleSheet(
            "QSlider::groove:horizontal { border: 1px solid #bbb; background: white; "
            "height: 10px; border-radius: 4px; } QSlider::sub-page:horizontal { "
            "background: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1, stop: 0 #66e, "
            "stop: 1 #bbf); background: qlineargradient(x1: 0, y1: 0.2, x2: 1, y2: "
            "1, stop: 0 #bbf, stop: 1 #55f); border: 1px solid #777; height: 10px; "
            "border-radius: 4px; } QSlider::add-page:horizontal { background: #fff; "
            "border: 1px solid #777; height: 10px; border-radius: 4px; } "
            "QSlider::handle:horizontal { background: qlineargradient(x1:0, y1:0, "
            "x2:1, y2:1, stop:0 #eee, stop:1 #ccc); border: 1px solid #777; width: "
            "13px; margin-top: -2px; margin-bottom: -2px; border-radius: 4px; } "
            "QSlider::handle:horizontal:hover { background: qlineargradient(x1:0, "
            "y1:0, x2:1, y2:1, stop:0 #fff, stop:1 #ddd); border: 1px solid #444; "
            "border-radius: 4px; } QSlider::sub-page:horizontal:disabled { "
            "background: #bbb; border-color: #999; } "
            "QSlider::add-page:horizontal:disabled { background: #eee; border-color: "
            "#999; } QSlider::handle:horizontal:disabled { background: #eee; border: "
            "1px solid #aaa; border-radius: 4px; }");
    push_button_exit_in_menu->setGeometry(
            {this->width() - push_button_exit_in_menu->geometry().width() - 10, 25,
...
void Room::init_timers() {
    ...
    connect(this, SIGNAL(signal_close_room()), this_window_have_focus_timer,
            SLOT(stop()));
...
void Room::keyPressEvent(QKeyEvent *apKeyEvent) {
    ...
    if (this->hasFocus() &&
        ...
        if (apKeyEvent->key() == Qt::Key_Y &&
            animation_scene->sleep.xl <= local_player->x() &&
            local_player->x() <= animation_scene->sleep.xr &&
            animation_scene->sleep.yl <= local_player->y() &&
            local_player->y() <= animation_scene->sleep.yr) {
            local_player->setPos(x_very_far, y_very_far);
            local_player->saturation -= 100;
            local_player->state = AnimateState::Standing;
        }
        if (apKeyEvent->key() == Qt::Key_Y &&
            animation_scene->want_or_dont_eat.xl <= local_player->x() &&
            local_player->x() <= animation_scene->want_or_dont_eat.xr &&
            animation_scene->want_or_dont_eat.yl <= local_player->y() &&
            local_player->y() <= animation_scene->want_or_dont_eat.yr) {
        ...
        if ((apKeyEvent->key() == Qt::Key_End || apKeyEvent->key() == 16777216) &&
            local_player->x() == x_very_far && local_player->y() == y_very_far) {
            local_player->setPos(place_to_leave_the_house.first,
                                 place_to_leave_the_house.second);
        }
...
void Room::add_video() {
    ...
    input_form->setLabelText(
            "Введите ссылку на видео, находящееся на yandex.disk");
