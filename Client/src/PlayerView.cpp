...
void PlayerView::paint(QPainter* painter,
  ...
  if (this->direction == "right") {
    QPixmap icon = Pixmaps[static_cast<int>(state)].first.copy(
        ...
        csd().height);
    icon =
        icon.transformed(QTransform().scale(-1, 1).translate(icon.width(), 0));
    painter->drawPixmap(0, 0, icon);
  } else {
    painter->drawPixmap(0, 0, Pixmaps[static_cast<int>(state)].first,
                        (csd().offset * current_frame + csd().border), 0,
